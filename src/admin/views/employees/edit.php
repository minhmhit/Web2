        ...
        <form method="POST" action="admin.php?page=employees&action=edit&id=<?= $employee['EmployeeID'] ?>" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Quyền <span class="text-red-500">*</span></label>
                        <select name="PermissionID" required
                                class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                            <option value="1" <?= $employee['PermissionID'] == 1 ? 'selected' : '' ?>>Quản trị viên</option>
                            <option value="2" <?= $employee['PermissionID'] == 2 ? 'selected' : '' ?>>Nhân viên</option>
                            <option value="3" <?= $employee['PermissionID'] == 3 ? 'selected' : '' ?>>Nhân viên bán hàng</option>
                        </select>
                    </div>
